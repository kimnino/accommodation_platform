...
| Outbound Port (명령) | `Persist{Domain}Port` | `PersistAccommodationPort`, `PersistRoomPort` |
| Outbound Port (조회) | `Load{Domain}Port` | `LoadAccommodationPort`, `LoadRoomPort` |
| Outbound Port (외부) | `{Domain}Client` | `SupplierClient` |
...
### 다국어(Translation) 패턴
- 번역 가능한 엔티티는 `{Entity}TranslationJpaEntity` 테이블로 분리
- 언어코드는 **소문자**로 저장 (`ko`, `en`, `ja`)
- 플랫폼 초기 지원 언어: 한국어(`ko`), 영어(`en`), 일본어(`ja`)
- 숙소별 지원 언어는 `accommodation_supported_locale` 테이블로 관리

### 이미지 저장
- 이미지는 **상대경로**만 저장 (예: `/accommodation/exterior/20260420102838.png`)
- 이미지 저장소 변경 시 base URL만 교체하면 되도록 설계

### JPA Entity 주석
- `@Entity` 클래스 상단에 테이블 설명 주석 필수
- 모든 필드에 한글 설명 주석 (`/** */`) 필수

### 수정 승인 구조
- 파트너가 데이터를 수정할 때는 즉시 반영하지 않고 **수정 요청(ModificationRequest)** 생성
- 관리자가 승인하면 실제 데이터에 반영, 거절 시 사유와 함께 반려
- 수정 데이터는 JSON으로 저장하여 유연성 확보

### 검색 쿼리 패턴
- 동적 검색 쿼리는 **QueryDSL** 사용 (JPQL 문자열 조합 금지)
- SELECT 서브쿼리 대신 **2단계 배치 조회**: 1단계 ID 목록 조회 → 2단계 IN절 배치 (이미지, 최저가 등)
- 가격 필터 등 조건은 반드시 **WHERE절에서 처리** (Java 후처리 금지 — 페이징 정확성)

### 도메인 서비스 Bean 관리
- 도메인 서비스는 **순수 Java** 유지 (`@Service` 금지)
- `DomainServiceConfig`에서 `@Bean`으로 등록 (프레임워크 무의존 원칙)
